...
We will install `numpy` in the container. Create a file called `python-bookworm.def` (you can use the command `touch python-bookworm.def`) and fill it with the following text (you can use a terminal text editor like `nano` or `vim`):
